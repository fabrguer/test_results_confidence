...
def CI_vs_samples(distribution, samples_list, samples_to_plot):
    ...
    for i, samples in enumerate(samples_list):
        ...
        if samples in samples_to_plot:
            ...
            ax[0].hist(data_avgs, bins=50, label=label, color=color);
            qqplot(np.array(data_avgs), fit=True, line='45', ax=ax[1], 
    ...
    ax[2].set_xlabel("n")
    ax[2].set_ylabel("CI score")
    ax[2].plot(samples_list, CI_df['CI_norm_score'].values, 'o', 
               label="Normal Approx (Eq.2)", color="crimson")
    ax[2].plot(samples_list, CI_df['CI_t_score'].values, 'o', 
               label="T-Distr Approx (Eq.3)", color="blue")
    ax[2].plot(samples_list, len(samples_list)*[0.95], '--', 
